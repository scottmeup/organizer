...
export async function updateShoppingItemRow(id, input) {
  const existing = await getShoppingItemRow(id);
  if (!existing) return null;
  return (await getPool().query('update shopping_items set title=$2, quantity=$3, checked=$4, updated_at=now() where id=$1 returning *', [id, input.title || existing.title, input.quantity ?? existing.quantity, input.checked ?? existing.checked])).rows[0];
}
