...
def fetch_payload():
    try:
        ...
        return {'profile': 'default', 'generatedAt': datetime.utcnow().isoformat() + 'Z', 'sections': [{'type': 'calendar', 'title': 'Calendar', 'items': [{'time': '09:00', 'title': 'Fallback event'}]}, {'type': 'tasks', 'title': 'Tasks', 'items': [{'state': 'open', 'title': 'Fallback task'}]}, {'type': 'shopping', 'title': 'Shopping', 'items': [{'checked': False, 'title': 'Fallback item'}]}]}
...
def build_preview_image():
    ...
    draw.text((40, 110), f'Generated: {payload.get("generatedAt")}', fill='black')
    y = 150
    for section in payload.get('sections', []):
        draw.text((40, y), section.get('title', 'Section'), fill='black')
        y += 30
        for item in section.get('items', [])[:5]:
            line = ' '.join(str(v) for v in item.values() if v is not None)
